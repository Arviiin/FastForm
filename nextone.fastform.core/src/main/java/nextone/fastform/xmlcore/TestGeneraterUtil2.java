...
public class TestGeneraterUtil2 {
			...
			List<Element> childElements= rootElement.elements();
			  for(Element childElement:childElements){
			   if(childElement.getName().equals("fields")){
				 ...
				 if(compon.get("inputType").equals("TEXT")){
	...
	public static Map getDataForFormBean(String XmlPath) throws DocumentException{
	    ...
	    for (Element childElement : childElements) {
	    	...
	    	case "fields":
				...
				data.put("entityBeanPackage", fieldsElement.attributeValue("connectpath"));
				String connectXmlPath=fieldsElement.attributeValue("connectXmlPath");
	...
	public static Map getDataForEntityBean(String XmlPath) throws NoSuchFieldException, SecurityException, DocumentException{
		...
		Map data =null;
			...
			getFiledOfEntityBeanDateModel(XmlPath, compons, mustCompons, codeCompons, switchCompons, multiCompons, maxlengthCompons);
	...
	public static Map getDataForSearchForm(String XmlPath) throws Exception{
		...
		data.put("prePackage", prePackage);
			...
			for(Element EntitychildElement:EntitychildElements){
				...
				getFiledOfEntityBeanDateModel(connectionXmlPath, compons, mustCompons, codeCompons, switchCompons, multiCompons, maxlengthCompons);
				//递归获取父类的数据
	...
	/*	String superclass = XmlPath.getSuperclass();
		...
		String superXmlPath = rootElement.attributeValue("extendsBaseEntity");
			// 拿到标注了FastEntity注解的实体类的父类(比如CompanyBean的父类BaseEntityBean)里的所有字段,遍历每个字段,若字段上有input注解则解析并将数据放入集合
			getFiledOfEntityBeanDateModel(superXmlPath, compons, mustCompons, codeCompons, switchCompons, multiCompons, maxlengthCompons);
			//递归获取父类的数据
	 ...
	 * @throws DocumentException 
	 */
	protected static void getFiledOfEntityBeanDateModel(String XmlPath,List compons,List mustCompons,List codeCompons,List switchCompons,List multiCompons,List maxlengthCompons) throws NoSuchFieldException, SecurityException, DocumentException {
		//1.获得解析器
		SAXReader saxReader=new SAXReader();
		//2.获得document文档对象
		Document doc = saxReader.read(XmlPath);
